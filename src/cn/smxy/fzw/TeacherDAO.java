...
//		Connection conn = connection.getConnection();	//1、获取到了数据库userdb连接
	...
	public List<Teacher> queryAll() throws Exception{
		...
		List<Teacher> users=new ArrayList<Teacher>();  //多态
		while(rSet.next()) { //while
			String tid = rSet.getString(1);  //用getString()获取第1个字段的值
			String tname = rSet.getString(2);  //用getString()获取第2个字段的值
	...
	public List<Teacher> queryAllByCondition(String contition) throws Exception{
		...
		while(rSet.next()) { //while
			...
			String tpwd = rSet.getString(3);  //用getString()获取第3个字段的值
			Teacher user =new Teacher(tid, tname, tpwd);
			users.add(user);	
		}
		return users;
